...
QBCore.Functions.CreateCallback('qb-fuel:server:refillVehicle', function(src, cb, litres, paymentMethod, fuelType)
    local Player = exports['qb-core']:GetPlayer(src)
    if not Player then return cb(false) end
    litres = math.floor(tonumber(litres) or 0)
    if litres <= 0 or litres > 100 then return cb(false) end

    local moneyType = paymentMethod == 'bank' and 'bank' or Config.MoneyType
    local finalPrice = litres * Config.FuelPrice * getFuelTypeMultiplier(fuelType)
    if (Player.PlayerData.money[moneyType] or 0) >= finalPrice then
        cb(Player.RemoveMoney(moneyType, finalPrice, 'refuel-vehicle'))
    else
        cb(false)
    end
end)
