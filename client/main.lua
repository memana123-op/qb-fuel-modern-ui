...
local getFuelTypeMultiplier = function(fuelType)
    if not Config.FuelTypes or not Config.FuelTypes[fuelType] then
        return 1.0
    end
...
local nozzleToVehicle = function(veh)
    ...
    CreateThread(function()
        Wait(450)
        if CurrentVehicle == veh and Entity(veh).state.nozzleAttached and showFuelMenu then
            showFuelMenu()
        end
    end)
end

local refillVehicleFuel = function(liter, paymentMethod, fuelType)
    if not CurrentPump then return end
    liter = math.floor(tonumber(liter) or 0)
    if liter <= 0 then return end
    ...
    local currentFuel = math.floor(exports['qb-fuel']:GetFuel(veh) or 0)
    local maxLiter = math.max(0, 100 - currentFuel)
    if maxLiter <= 0 then return QBCore.Functions.Notify(Lang:t('error.vehicle_full'), 'error') end

    liter = math.min(liter, maxLiter)
    local moneyType = paymentMethod == 'bank' and 'bank' or Config.MoneyType
    local finalPrice = liter * Config.FuelPrice * getFuelTypeMultiplier(fuelType)
    if (QBCore.PlayerData.money[moneyType] or 0) < finalPrice then return QBCore.Functions.Notify(Lang:t('error.no_money'), 'error') end
    ...
    QBCore.Functions.Progressbar('fueling_vehicle', Lang:t('progress.refueling'), Config.RefillTimePerLitre * liter * 1000, false, true, {
        ...
        local success = QBCore.Functions.TriggerCallback('qb-fuel:server:refillVehicle', liter, paymentMethod, fuelType)
        if not success then return QBCore.Functions.Notify(Lang:t('error.no_money'), 'error') end
        removeObjects()
        exports['qb-fuel']:SetFuel(veh, math.min(100, math.floor(exports['qb-fuel']:GetFuel(veh) or 0) + liter))
        QBCore.Functions.Notify(Lang:t('success.refueled'), 'success')
        ClearPedTasks(ped)
    end, function()
        removeObjects()
...
showFuelMenu = function()
    ...
    local vehicleName = GetLabelText(modelName)
    ...
    SendNUIMessage({
        ...
        vehicle = {
            model = vehicleName,
            imageModel = string.lower(modelName),
            plate = GetVehicleNumberPlateText(veh),
            speed = math.floor(GetEntitySpeed(veh) * 3.6),
            engineTemperature = math.floor(GetVehicleEngineTemperature(veh) or 96)
        }
    })
...
local setUpTarget = function()
    if TargetsReady then return end
...
RegisterNuiCallback('refill', function(data, cb)
    if not data or not data.liter then
        cb(false)
        return
    end

    local liter = math.floor(tonumber(data.liter) or 0)
    if liter <= 0 then
        cb(false)
        return
    end
    ...
    hideFuelMenu()
    refillVehicleFuel(liter, data.paymentMethod, data.fuelType)
    cb('ok')
end)
